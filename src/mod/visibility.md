...
По умолчанию, элементы модуля являются закрытыми,
но это можно изменить добавив переключатель `pub`.
Только общедоступные элементы модуля могут быть доступны за пределами его области видимости.

```rust,editable
// Модуль по имени `my_mod`
mod my_mod {
    // Все элементы модуля по умолчанию являются закрытыми.
    fn private_function() {
    ...
    // Закрытые элементы модуля доступны другим элементам
    // данного модуля.
...
fn main() {
    ...
    // Общедоступные элементы, включая те, что находятся во вложенном модуле,
    ...
    // Закрытые элементы модуля не доступны напрямую,
    // даже если вложенный модуль является открытым:
    ...
    // Ошибка! Модуль `private_nested` является закрытым
