...
enum List {
    // Cons: Упорядоченная стопка, которая хранит элемент
...
// Способы могут быть присоединены к перечислению
impl List {
    ...
    fn len(&self) -> u32 {
        ...
        // поскольку поведение этого способа зависит от исхода `self`
        // `self` имеет вид `&List`, а `*self` имеет вид `List`, сопоставление на
        // определенном виде `T` предпочтительнее, чем сопоставление по ссылке `&T`
        match *self {
            // Мы не можем завладеть `tail`, т.к. `self` заимствован;
            // вместо этого возьмём ссылку на `tail`
            Cons(_, ref tail) => 1 + tail.len(),
            // Основной случай: Пустой список имеет нулевую длину
            Nil => 0
        }
    }
    ...
    fn stringify(&self) -> String {
        match *self {
            Cons(head, ref tail) => {
                ...
                // размещённую в куче, вместо вывода на окно вывода
...
[`Box`](std/box.html) и [`способы`](fn/methods.html)
