...
До появления `?`, подобная возможности  была доступна через макрос `try!`.
Сейчас советуется использовать приказчик `?`, но вы до сих пор можете найти `try!`, когда 
просматриваете старая рукопись. Функция `multiply` из предыдущего примера с использованием `try!` будет 
