...
Способы - это функции, прикреплённые к объектам. Эти способы 
имеют допуск к данным объекта и другим его способам через 
...
impl Rectangle {
    ...
    fn area(&self) -> f64 {
        // `self` даёт допуск к полям структуры через приказчик точка
    ...
    // Этот способ требует чтобы вызываемый объект был изменяемым
...
impl Pair {
    // Этот способ "съедает" ресурсы вызываемого объекта
...
fn main() {
    ...
    // Хорошо, изменяемый объект может вызывать изменяемые способы
